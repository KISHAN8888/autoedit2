...
class VideoSummarizer:
    ...
    def __init__(self, azure_openai_key: str, azure_openai_endpoint: str, 
        ...
        if self.tts_engine == 'murf':
            ...
            self.vibe_voice_client = VibeVoice() # add vibe voice client here
            logger.info("Using VibeVoice for audio generation.")
        else:
            raise ValueError(f"Unsupported TTS engine: {tts_engine}. Choose 'murf' or 'gtts' or 'vibe_voice'.")
    ...
    async def optimize_segment_with_context_async(self, segment: SRTSegment, video_context: Dict, 
        ...
        You are an expert video script editor and writer. Your task is to optimize the 'CURRENT SEGMENT' for clarity, creativity and brevity while maintaining the original meaning and tone. Add 10x value to the segment!
        Make it engaging, interesting, and informative.
