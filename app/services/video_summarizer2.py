...
# <<< CHANGED >>>: The Pydantic model for the LLM response is now more powerful.
...
class SentenceGroup:
    ...
    index: int
    grouped_text: str # This will now be the polished text from the LLM
...
class FFmpegProcessor:
    ...
    # (No changes to FFmpegProcessor, it is included for completeness)
...
class VideoSummarizer:
    ...
    async def optimize_all_segments_async(self, segments: List[SRTSegment]) -> List[OptimizedSegment]:
        ...
        # Placeholder for light cleanup. In a real scenario, you could use an LLM for more advanced per-segment optimization.
        optimized_segments = [
            OptimizedSegment(
                original=segment, 
    ...
    async def group_segments_into_sentences_async(self, optimized_segments: List[OptimizedSegment]) -> List[SentenceGroup]:
        """
        NEW & IMPROVED: Uses an LLM to group segments into sentences AND return the polished, merged text directly.
        """
        ...
        # <<< CHANGED >>>: The schema now includes the 'polished_sentence'.
        json_schema = SentenceGroupingResponse.model_json_schema()

        # <<< CHANGED >>>: The prompt is updated to ask for the polished text directly.
        grouping_prompt = f"""
        ...
        try:
            ...
            for i, group_info in enumerate(grouping_data.sentences):
                ...
                # <<< CHANGED >>>: We now directly use the polished sentence from the LLM.
...
# In VideoSummarizer class
    ...
    async def generate_tts_audio_async(self, text: str, temp_dir: str, voice_id: str = 'en-IN-arohi', 
        ...
        # <<< FIX 1: Standardize the output filename within the function >>>
        # Both Murf and gTTS will now save to the same relative path.
        # The '.mp3' extension is more common for both.
        ...
        if self.tts_engine == 'murf':
                ...
                try:
                    ...
                    def generate_murf_tts():
                        res = self.murf_client.text_to_speech.generate(
                            text=text,
                            voice_id=voice_id,
                            format='mp3' # Request MP3 format for consistency
                        )
                        return res.audio_file, res.audio_length_in_seconds
                    ...
                    logger.info(f"Successfully saved Murf TTS audio to {output_path}")
                    # Use ffprobe to get the most accurate duration
            ...
            try:
                ...
                def generate_gtts():
                    tts = gTTS(text=text, lang='en')
                    tts.save(output_path) # Saves directly to the correct standardized path
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            async def process_single_group(group: SentenceGroup):
                async with semaphore:
                    ...
                    # <<< FIX 2: Call TTS with the DIRECTORY, and UNPACK the (path, duration) tuple >>>
                    generated_audio_path, new_audio_duration = await self.generate_tts_audio_async(
                        group.grouped_text, segment_temp_dir
                    )
                    
                    # Now, new_audio_duration is a float, so this comparison is safe.
                    ...
                    # Use the actual path returned by the TTS function
...
#     INPUT_VIDEO_PATH = r"C:\Users\kisha\Downloads\hallucinations.mp4"
#     SRT_FILE_PATH = r"C:\Users\kisha\Downloads\hallucinations.srt"
...
#     # To run this, you need to set your environment variables:
#     # export AZURE_OPENAI_KEY="your_key"
#     # export AZURE_OPENAI_ENDPOINT="your_endpoint"
#     # And install dependencies: pip install openai pydantic gtts
#     asyncio.run(main())
