...
class VideoSummarizer:
    ...
    async def process_video_segments_async(self, input_video: str, optimized_segments: List[OptimizedSegment]) -> str:
        ...
        Uses a temporary directory for intermediate files and moves the final
        output to a persistent temporary file to prevent premature deletion.
        """
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if video_duration == 0.0:
                    ...
                    elif os.path.getsize(input_video) == 0:
                        raise ValueError(f"Input video file is empty: {input_video}")
                    else:
                        raise ValueError(f"Could not read video duration. File may be corrupted or in unsupported format: {input_video}")
                
                # Process segments with limited concurrency
                semaphore = asyncio.Semaphore(2)  # Limit concurrent video processing
                
                async def process_single_segment(i, opt_seg):
                    async with semaphore:
                        # Create segment-specific temp directory
                        segment_temp_dir = os.path.join(temp_dir, f"segment_{i}")
                        os.makedirs(segment_temp_dir, exist_ok=True)
                        
                        # Generate TTS audio
                        audio_path, actual_audio_duration = await self.generate_tts_audio_async(
                            opt_seg.optimized_text, segment_temp_dir
                        )
                        
                        if not audio_path or actual_audio_duration <= 0:
                            logger.warning(f"Skipping segment {i} due to audio generation failure.")
                            return None
                        
                        # Extract and process video segment using sync FFmpeg
                        ...
                        speed_multiplier = original_video_seg_duration / actual_audio_duration if actual_audio_duration > 0 else 1
                        ...
                        try:
                            ...
                            loop = asyncio.get_event_loop()
                            returncode, stdout_sync, stderr_sync = await loop.run_in_executor(
                ...
                tasks = [process_single_segment(i, opt_seg) for i, opt_seg in enumerate(optimized_segments)]
                ...
                # Define the output path INSIDE the temporary directory for processing
                internal_output_video = os.path.join(temp_dir, "final_output.mp4")
                
                # Combine all processed segments into the internal temporary video file
                await self._robust_combine_segments_async(video_segments, audio_segments, internal_output_video, temp_dir)
                
                # Create a persistent temporary file path that will NOT be auto-deleted.
                # The calling script is responsible for managing this file after completion.
                with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as dest_f:
                    persistent_output_path = dest_f.name
                
                # Move the successfully created video from the auto-deleting directory to the persistent path
                shutil.move(internal_output_video, persistent_output_path)
                
                logger.info(f"Video processing complete. Final output moved to persistent temp file: {persistent_output_path}")
                
                # Return the path to the NEW, safe location
